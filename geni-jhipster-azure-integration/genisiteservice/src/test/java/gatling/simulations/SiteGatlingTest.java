 ...
 */
public class SiteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all sites").get("/services/genisiteservice/api/sites").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new site")
                        .post("/services/genisiteservice/api/sites")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created site").get("/services/genisiteservice${new_site_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created site").delete("/services/genisiteservice${new_site_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Site entity").exec(scn);
